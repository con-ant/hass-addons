...
| `GET /usage` | the Claude subscription usage snapshot the package's `sensor.claude_usage_*` entities read (README "Claude subscription usage"): `session` / `weekly` / `weekly_scoped` (`used_percent`, `resets_at`, `severity`, `is_active`; weekly also `breakdown` by surface), `weekly_scoped_all`, `extra_usage`, and the poll health `ok`, `stale`, `has_data`, `last_success`, `last_attempt`, `last_error`, `last_error_detail`, `age_s`, `credential_expires_at`, `subscription_type`. Always `200` while `enable_usage_sensors` is on — a failed refresh keeps the last good values with `stale: true`; `404 usage_disabled` when off. Never carries the OAuth token |
...
| `sensor.claude_usage_*`, `binary_sensor.claude_usage_stale` | package REST sensors polling `GET /usage` every `usage_poll_interval` s (add-on option `enable_usage_sensors`, default on); registry-backed | your **whole claude.ai subscription's** usage: 5-hour session %, weekly %, weekly per-model %, the reset timestamps, extra-usage spend (major units), the weekly breakdown by surface as each surface's share of the week's usage (`sensor.claude_usage_weekly_claude_code` / `_chats` / `_cowork` / `_other`). Not job spend — that is `sensor.claude_jobs_monthly_cost`. Full list and the token-freshness caveat: README "Claude subscription usage" |
...
| `binary_sensor.claude_usage_stale` is on, `sensor.claude_usage_last_success` attribute `last_error: unauthorized` | the claude.ai access token expired (`credential_expires_at` is in the past) — the CLI renews it only when it calls the model. Type in the terminal, run any job, or schedule/enable the shipped `usage-keepalive` job (README "Claude subscription usage"). If `credential_expires_at` is in the future, the login was revoked: `/login` |
| `sensor.claude_usage_*` all `unavailable`, `last_error: no_credentials` | no claude.ai OAuth credential store (`/homeassistant/.claudecode/.credentials.json`): the install is not logged in, or uses an API key (usage figures exist only for claude.ai subscriptions) |
| `last_error: malformed_body` | `api.anthropic.com` answered with something that is not the usage document (its shape is undocumented and may change); the last good values are kept. Report it with the add-on version |
