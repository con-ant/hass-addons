...
**Subscription usage sensors (rev con.19).** The same package carries a second `rest:` resource, `GET /usage`, whose figures are not about jobs at all: the account-wide claude.ai plan limits the CLI's own `/usage` screen shows (5-hour session, weekly, weekly per-model, extra usage, weekly breakdown by surface). The endpoint's tick (duty 6) polls `api.anthropic.com/api/oauth/usage` every `usage_poll_interval` s with the interactive session's OAuth access token, read through the same newest-wins credential view the job config dir uses (`jobcommon.oauth_credentials()`, read-only). Rules: the token is **read, never refreshed** (a second refresher races the CLI's locked refresh and rotates the refresh token from under it); a poll runs on its own thread, single flight, 10 s timeout, so neither the tick nor a handler ever waits on the internet; a failed poll (401 once the ~8 h token expired on an idle install, network, malformed body) keeps the **last good values** and flags them `stale` with `last_success` — zeros are never published; failures log on transition only; the token never reaches the snapshot, the package or the log. The package's usage entities are registry sensors with `availability:` guards, `device_class: timestamp` for resets, `state_class: measurement` for percentages, the breakdown as attributes plus per-surface template sensors, and `binary_sensor.claude_usage_stale` as the honesty signal. The only refresh path is the CLI's own model call, so freshness on an idle install is a *job*: the shipped `usage-keepalive` (haiku, two turns, no tools), scheduled by the user or spawned by the endpoint through the ordinary run path when `usage_keepalive_job` names it — at most hourly and only while the token's own `expiresAt` is past (a rejected-but-unexpired token is a revoked login). The block is omitted from the rendered package when `enable_usage_sensors` is off. The upstream endpoint is undocumented; unknown keys are ignored, missing ones tolerated.
...
Rev con.19 adds three for the subscription usage sensors (§4.10), gated behind `enable_job_endpoint`: `enable_usage_sensors` (`bool`, `true` — it costs nothing when the endpoint runs), `usage_poll_interval` (`int(60,3600)`, 300; promoted straight to an option because the poll cadence is a per-account spend-visibility choice, not a constant the design can pick), and `usage_keepalive_job` (`str?`, empty — a spend decision, so off by default).
