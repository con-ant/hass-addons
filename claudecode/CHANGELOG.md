...
### Added
- **Home Assistant package for Claude Jobs, generated by the add-on.** With
  `enable_job_endpoint: true`, every start writes `/homeassistant/claudecode_jobs.yaml`
  (hostname, port and endpoint token filled in) and the scheduling blueprint
  `blueprints/automation/claudecode/schedule.yaml`. Add
  `homeassistant: packages: claudecode_jobs: !include claudecode_jobs.yaml` to
  `configuration.yaml` and restart Home Assistant once; that yields
  `rest_command.claude_job_run` / `claude_job_set_enabled` / `claude_job_republish`, the
  registry-backed anchor `sensor.claude_jobs_attention` (polls the endpoint every 60 s;
  state = stale + failed jobs, `unavailable` = endpoint down), the statistics-grade
  `sensor.claude_jobs_monthly_cost`, an attention alarm with its clear-down, and a
  republish-on-start automation. Later re-renders (rotated token) reload RESTful commands
  and REST sensors automatically. If you used the interim `rest_command:` block from
  1.2.65-con.9, delete it before restarting.
- On git-backed config directories the renderer adds `/claudecode_jobs.yaml` to
  `.git/info/exclude` and warns at boot if the file is already tracked (the token would be
  in your history: rotate it). Docs (`docs/JOBS.md`): onboarding, entities, blueprint scheduling,
  run-now dashboard example, token rotation, git callout, an external dead-man recipe for
  monitoring HA itself, and a troubleshooting table. Design: `docs/DESIGN-claude-jobs.md`
  §4.10, §6.4.
